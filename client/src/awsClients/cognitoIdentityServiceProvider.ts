...
const createSecretHash = (username: string) => 
    crypto.createHmac('SHA256', "alm159kg846j3g7nc6aab7b5g524e22la2evnji2ceao48dkea2")
...
export const confirmSignUp = (Username: string, ConfirmationCode: string) => {
    const SecretHash = createSecretHash(Username);
    return cognitoIdentityServiceProvider.confirmSignUp({
        ClientId,
        ConfirmationCode,
        Username,
        SecretHash,
    ...
    .then(({ AuthenticationResult }) => {
        if (AuthenticationResult) {
            if (AuthenticationResult.IdToken) {
                const { header: { kid } } = JWT.decode(AuthenticationResult.IdToken, { complete: true}) as DecodeResult;
                const publicKey = find(jwks.keys, { kid }) as JWK;
                const { sub, aud, iss, token_use, exp } = JWT.verify(AuthenticationResult.IdToken, jwkToPem(publicKey), { algorithms: ['RS256'] }) as JwtPayload;
                ...
                if (aud !== ClientId) {
                    throw new Error(`Log in returned incorrect audience ${aud}, expected ClientId ${ClientId}`);
                }

                const expectedIss = `https://cognito-idp.us-east-1.amazonaws.com/us-east-1_QWBKwgoCb`;
                if (iss !== expectedIss) {
                    throw new Error(`Log in returned incorrect iss ${iss}, expected ${expectedIss}`);
                }

                if (token_use !== 'id') {
                    throw new Error(`Log in returned incorrect token use ${token_use}, expected id`);
                }
